...
data class CountryData(
    ...
    companion object {
        ...
        fun delete(id: Int): Int = CountryData(id = id).delete()

        fun getCountryId(country: String): Int {
            val query: List<QueryResult<CountryData>> =
                queryDatabase(whereArgs = WhereArgs("${CountryColumns.NAME.name} = ?", listOf(country)))
            if (query.isEmpty()) {
