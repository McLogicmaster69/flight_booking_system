...
data class StaffData(
    ...
    override fun debugData() {
        println(
            "Staff data: (\"$id\", \"$firstName\", \"$lastName\", \"$positionId\", \"$loginId\", \"$currentLocation\", \"$homeId\")",
        )
    }
    ...
    companion object {
        ...
        fun queryDatabase(id: Int): List<QueryResult<StaffData>> =
            queryDatabase(whereArgs = WhereArgs("${StaffColumns.ID.name} = ?", listOf(id)))
        ...
        fun delete(id: Int): Int = StaffData(id = id).delete()

        fun queryByLogIn(email: String): List<QueryResult<StaffData>> {
