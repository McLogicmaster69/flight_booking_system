...
object PlaneModelColumns {
    ...
    val MANUFACTURER_ID =
        Column<Int>("manufacturer_id", "INTEGER NOT NULL REFERENCES ${ManufacturerData.EMPTY.tableName}(id)")
...
data class PlaneModelData(
    ...
    override fun debugData() {
        println(
            "Plane model data: (\"$id\", \"$name\", \"$manufacturerId\", \"$capacity\", \"$pilots\", \"$attendants\")",
        )
    }
    ...
    companion object {
        ...
        fun queryDatabase(id: Int): List<QueryResult<PlaneModelData>> =
            queryDatabase(whereArgs = WhereArgs("${PlaneModelColumns.ID.name} = ?", listOf(id)))
        ...
        fun delete(id: Int): Int = PlaneModelData(id = id).delete()
    }
}
