...
data class ScheduleData(
    ...
    override val tableAdditionalSQL = "UNIQUE (${ScheduleColumns.ROUTE_ID.name}, ${ScheduleColumns.MODEL_ID.name}, ${ScheduleColumns.START_DATE.name}, ${ScheduleColumns.DAY_GAP.name}, ${ScheduleColumns.TIME.name})"
    ...
    override fun debugData() {
        println("Schedule data: (\"$id\", \"$routeId\", \"$modelId\", \"$active\", \"$startDate\", \"$dayGap\", \"$time\")")
    }
    ...
    fun updateFlights() {
        ...
        val flights : List<QueryResult<FlightData>> = FlightData.queryDatabase(
            whereArgs = WhereArgs(
        ...
        val nextPlannedFlightDate : LocalDate = 
            if (flights.isEmpty()) startDate 
            else flights.first().dataClass.date.plusDays(dayGap.toLong())

        val nextActualFlightDate : LocalDate =
            if (nextPlannedFlightDate.isBefore(LocalDate.now())) LocalDate.now().plusDays(
                dayGap - (Duration.between(
                    nextPlannedFlightDate.atStartOfDay(),
                    LocalDate.now().atStartOfDay()
                ).toDays() % dayGap)
            )
            else nextPlannedFlightDate
    ...
    companion object {
        ...
        fun queryDatabase(id : Int) : List<QueryResult<ScheduleData>> = queryDatabase(whereArgs = WhereArgs("${ScheduleColumns.ID.name} = ?", listOf(id)))
        ...
        fun delete(id : Int) : Int {
            return ScheduleData(id = id).delete()
        }
