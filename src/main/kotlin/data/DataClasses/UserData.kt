...
data class UserData(
    ...
    override fun debugData() {
        println(
            "User data: (\"$id\", \"$firstName\", \"$lastName\", \"$verifiedAccount\", \"$loyaltyPoints\", \"$loginId\")",
        )
    }
    ...
    companion object {
        ...
        fun queryByLogIn(email: String): List<QueryResult<UserData>> {
        ...
        fun queryByToken(token: String): List<QueryResult<UserData>> {
            val joinArgs: MultipleJoinArgs =
                MultipleJoinArgs(
                    listOf(
                        JoinArgs(
