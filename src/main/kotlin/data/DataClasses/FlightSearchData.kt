...
data class FlightSearchData(
    ...
    override fun debugData() {
        println(
            "Flight Search data: (\"$id\", \"$token\", \"$startDestination\", \"$endDestination\", \"$date\", \"$createdAt\")",
        )
    }
    ...
    companion object {
        ...
        fun delete(id: Int): Int = FlightSearchData(id = id).delete()
        ...
        fun createSearch(
            ...
            val flightSearch: FlightSearchData =
                FlightSearchData(
                    token = token,
        ...
        fun queryByToken(token: String): FlightSearchInfo? {
            val query: List<QueryResult<FlightSearchData>> = queryDatabase(token)
            if (query.isEmpty()) {
                return null
            }
        ...
        fun queryFlightPath(path: JourneyFlightTimePath): FlightSearchData? {
            val whereArgs: WhereArgs =
                WhereArgs(
                    whereClause = """
                        ${FlightSearchColumns.START_DESTINATION.name} = ?
                        AND ${FlightSearchColumns.END_DESTINATION.name} = ?
                        AND ${FlightSearchColumns.DATE.name} = ?
                    """,
                    whereArgs =
                        listOf(
            ...
            val query: List<QueryResult<FlightSearchData>> =
                queryDatabase(
                    whereArgs = whereArgs,
                )
        ...
        fun queryOrAddFlightPath(path: JourneyFlightTimePath): FlightSearchData {
