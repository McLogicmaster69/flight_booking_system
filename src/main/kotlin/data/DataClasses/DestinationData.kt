...
data class DestinationData(
    ...
    override val tableAdditionalSQL = """
        UNIQUE (
            ${DestinationColumns.COUNTRY_ID.name},
            ${DestinationColumns.CITY_NAME.name}
        )"""
    ...
    override val initialRows: List<DestinationData>
        get() =
            listOf(
                DestinationData(
                    countryId = CountryData.getCountryId("United Kingdom"),
                    cityName = "Luton",
                    timezoneId = TimezoneData.getTimezoneId("GMT"),
                ),
                DestinationData(
                    countryId = CountryData.getCountryId("United Kingdom"),
                    cityName = "Heathrow",
                ...
                DestinationData(
                    countryId = CountryData.getCountryId("Slovakia"),
                    cityName = "Bratislava",
                    timezoneId = TimezoneData.getTimezoneId("CET"),
                ),
                DestinationData(
                    countryId = CountryData.getCountryId("Japan"),
                    cityName = "Tokyo",
                    timezoneId = TimezoneData.getTimezoneId("JST"),
                ),
                DestinationData(
                    countryId = CountryData.getCountryId("Germany"),
                    cityName = "Berlin",
                    timezoneId = TimezoneData.getTimezoneId("CET"),
                ),
            )
    ...
    companion object {
        ...
        fun delete(id: Int): Int = DestinationData(id = id).delete()
        ...
        fun queryDatabase(
            ...
            val whereArgs: WhereArgs =
                WhereArgs(
                    whereClause = """
                        ${EMPTY.tableName}.${DestinationColumns.CITY_NAME.name} = ? 
                        AND ${CountryData.EMPTY.tableName}.${CountryColumns.NAME.name} = ?
                    """,
        ...
        fun queryDatabase(id: Int): List<QueryResult<DestinationData>> =
            queryDatabase(whereArgs = WhereArgs("${DestinationColumns.ID.name} = ?", listOf(id)))
        ...
        fun getDestinationName(id: Int): String {
            ...
            return "${query.first().dataClass.cityName} - ${query
                .first()
                .getColumn(
                    CountryData.EMPTY.tableName,
                    CountryColumns.NAME.name,
                )?.columnVal ?: ""}"
        }
        ...
        fun getDestinationNames(): List<String> {
            ...
            return queryDatabase(joinArgs).map { result ->
                "${result.dataClass.cityName} - ${result
                    .getColumn(
                        CountryData.EMPTY.tableName,
                        CountryColumns.NAME.name,
                    )?.columnVal ?: ""}"
        ...
        fun getDestinationId(destination: String): Int {
            val query: List<QueryResult<DestinationData>> =
                queryDatabase(whereArgs = WhereArgs("${DestinationColumns.CITY_NAME.name} = ?", listOf(destination)))
            if (query.isEmpty()) {
