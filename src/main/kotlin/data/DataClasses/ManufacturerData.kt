...
data class ManufacturerData(
    ...
    companion object {
        ...
        fun delete(id: Int): Int = ManufacturerData(id = id).delete()

        fun getManufacturerId(manufacturer: String): Int {
            val query: List<QueryResult<ManufacturerData>> =
                queryDatabase(whereArgs = WhereArgs("${ManufacturerColumns.NAME.name} = ?", listOf(manufacturer)))
            if (query.isEmpty()) {
