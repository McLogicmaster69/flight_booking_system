...
data class FlightData(
    ...
    override val tableAdditionalSQL = """
        UNIQUE (
            ...
            ${FlightColumns.DATE.name}, ${FlightColumns.TIME.name})
        """
        ...
        val pilots =
            model.pilots -
                staffAssignments
                    .filter {
                        StaffData
                            .queryDatabase(it.dataClass.staffId)
                            .first()
                            .dataClass.positionId == pilotId
                    }.size
                ...
                staffAssignments
                    .filter {
                        StaffData
                            .queryDatabase(it.dataClass.staffId)
                            .first()
                            .dataClass.positionId == attendantId
                    }.size
    ...
    companion object {
        ...
        fun queryDatabase(
            ...
            val whereClause =
                "(" + routeIds.joinToString(" OR ") { "${FlightColumns.ROUTE_ID.name} = ?" } +
        ...
        fun queryDatabase(
            ...
            queryDatabase(destinationArgs, dateTime.toLocalDate(), multipleJoinArgs, orderByArgs, limitArgs)

        fun queryDatabase(id: Int): List<QueryResult<FlightData>> =
            queryDatabase(whereArgs = WhereArgs("${FlightColumns.ID.name} = ?", listOf(id)))
        ...
        fun delete(id: Int): Int = FlightData(id = id).delete()
        ...
        private fun getJourneyPath(
            ...
            val query: List<QueryResult<FlightData>> =
                queryDatabase(
                    DestinationArgs(
                        locationId,
                    ...
                    queryDatabase(
                        ...
                        offsetTimeByZone(
                            LocalDateTime.of(result.dataClass.date, result.dataClass.time),
                            locationId,
                        ).isAfter(dateTime)
                    }
            ...
            for (flight in query) {
                val minutesBetweenFlights: Long =
                    Duration
                        .between(
                            dateTime,
                            offsetTimeByZone(
                ...
                val paths: List<JourneyFlightTimePath> =
                    getJourneyPath(
                        route,
                        offsetTimeByZone(
                            LocalDateTime.of(
                ...
                for (path in paths) {
                    output.add(
                        JourneyFlightTimePath(
                            ...
                            listOf(
                                LocalDateTime.of(flight.dataClass.date, flight.dataClass.time),
                            ) + path.localDateTimes,
        ...
        fun createFlight(
            ...
            val route: RouteData =
                RouteData.queryDatabase(routeId).firstOrNull()?.dataClass
                    ?: return CreateFlightResults(returnMessage = "Could not find route")
            val model: PlaneModelData =
                PlaneModelData.queryDatabase(modelId).firstOrNull()?.dataClass
                    ?: return CreateFlightResults(returnMessage = "Could not find plane model")
            val plane: PlaneData =
                PlaneData.getAvailablePlane(modelId, route.startDestination, date, time)
                    ?: return CreateFlightResults(returnMessage = "Could not find an available plane")
            ...
            val arrivalDateTime: LocalDateTime =
                LocalDateTime.of(date, time).plusMinutes(
                    route.duration.hour * 60L + route.duration.minute + 60L,
                )
            ...
            val staffResults: StaffAssignmentResults =
                AssignedFlightStaffData.assignStaffToFlight(
                    flightId,
                    model.pilots,
                    model.attendants,
        ...
        fun getIncompleteFlights(): List<QueryResult<FlightData>> {
            val pilotId: Int =
                StaffPositionData
                    .queryDatabase(StaffPositions.PILOT)
                    .first()
            ...
            val joinArgs: MultipleJoinArgs =
                MultipleJoinArgs(
                    listOf(
                        JoinArgs(
                            "INNER",
                            PlaneData.EMPTY.tableName,
                            FlightColumns.PLANE_ID.name,
                            PlaneColumns.ID.name,
                            PlaneColumns.COLUMN_NAMES,
                        ),
                        JoinArgs(
                            "INNER",
                            PlaneModelData.EMPTY.tableName,
                            PlaneColumns.MODEL_ID.name,
                            PlaneModelColumns.ID.name,
                            PlaneModelColumns.COLUMN_NAMES,
                            PlaneData.EMPTY.tableName,
                        ),
                        JoinArgs(
                            "LEFT",
                        ...
                        JoinArgs(
                            "LEFT",
                            StaffData.EMPTY.tableName,
                            AssignedFlightStaffColumns.STAFF_ID.name,
                            StaffColumns.ID.name,
                            StaffColumns.COLUMN_NAMES,
                            AssignedFlightStaffData.EMPTY.tableName,
                        ),
                    ),
                )

            val groupByArgs: GroupByArgs =
                GroupByArgs(
                    "${FlightData.EMPTY.tableName}.${FlightColumns.ID.name}, ${PlaneModelData.EMPTY.tableName}.${PlaneModelColumns.PILOTS.name}, ${PlaneModelData.EMPTY.tableName}.${PlaneModelColumns.ATTENDANTS.name}",
                    HavingArgs(
                        """
                    SUM(CASE WHEN ${StaffData.EMPTY.tableName}.${StaffColumns.POSITION_ID.name} = ? THEN 1 ELSE 0 END) < ${PlaneModelData.EMPTY.tableName}.${PlaneModelColumns.PILOTS.name}
                    OR
                    SUM(CASE WHEN ${StaffData.EMPTY.tableName}.${StaffColumns.POSITION_ID.name} = ? THEN 1 ELSE 0 END) < ${PlaneModelData.EMPTY.tableName}.${PlaneModelColumns.ATTENDANTS.name}
                    """,
                        listOf(pilotId, attendantId),
                    ),
                )
        ...
        fun assignStaffToIncompleteFlights() {
            println("Assigning staff to incomplete flights")
            val incompleteFlights: List<QueryResult<FlightData>> = getIncompleteFlights()
            println("Found ${incompleteFlights.size} incomplete flights")
            incompleteFlights.forEach { flight ->
